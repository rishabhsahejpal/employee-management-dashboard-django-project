	...
	const animations = {
		...
		showMessage : function(type){
			if (type == 'delete-permission')
				...
				return '<h2>Removing your project...</h2>'
		
			else if(type == 'update-project-list')
				return "<h2>Updating your projects\' list...</h2>"
		
			else if(type == 'update-project')
				return '<h2>Updating your project...</h2>'
		
			else if(type == 'adding-project')
				return '<h2>Adding your project...</h2>'
		...
		editProjectClick : function(){
			const edit = document.querySelectorAll('.edit-project');
			edit.forEach( (elem,i)=>{
				elem.addEventListener('click', event=>{
					event.preventDefault();
					this.showForm(elem);
		...
		cancelEditProjectClick : function(){
			...
			cancel.forEach( (elem,i)=>{
				elem.addEventListener('click', event=>{
					event.preventDefault();
					this.hideForm(elem);
					//Empty input text
					this.emptyTextInput( $(elem).siblings('input[type="text"]')[0] );
				});
			});
		},
		...
		saveEditProjectClick : function(context){
			...
			save.forEach(function(elem,i){
				elem.addEventListener('click',function(event){
					//Set DOM element and project_id to be used later
					context.DOM.elem_updating_projects = elem;
					context.VAR.project_id = elem.getAttribute('data-id')
		...
		update_db_ajax_call : function(text,context){
			...
			context.loadMessageIntoProjectsLoading('update-project');
		...
		deleteProjectClick : function(){
			...
			// Have to delete the 
			deleteProject.forEach( (elem,i)=>{
				elem.addEventListener('click', event=>{
					event.preventDefault();
					//Set the element to remove and set the id varaible
					this.DOM.project_to_remove = elem.closest('li.d-flex');
					//Update variable so they can be used later on
					this.VAR.project_id = elem.getAttribute('data-id');
					this.VAR.employee = elem.getAttribute('data-employee');
		...
		addProjectListing : function(context,text,id,assigned){
			...
			let project = `<li class='d-flex'>
						<h3 class="justify-content-between flex-column align-items-start">
							...
							<span><i class="fas fa-edit"></i>Assigned On: ${assigned}</span>
						</h3>
						<ul class="options">
